import { YexaaMoonOutragConst } from './yexaaMoonOutragConst';
import { YexaaLocalConstant } from './yexaaLocalConstant';

export class YexaaPanchangImpl {
  kyear!: number;
  kmon!: number;
  kday!: number;
  private yexaaMoonOutragConst: YexaaMoonOutragConst = new YexaaMoonOutragConst();
  constructor(private yexaaConstant: YexaaLocalConstant) {}
  d2r = Math.PI / 180;
  r2d = 180 / Math.PI;
  ...
  moon(jd: number) {
    ...
    for (let i = 0; i < 93; i++) {
      // outrage at length
      let arg =
        this.yexaaMoonOutragConst.corrMoon[i].mlcor * ml +
        this.yexaaMoonOutragConst.corrMoon[i].mscor * ms +
        this.yexaaMoonOutragConst.corrMoon[i].fcor * f +
        this.yexaaMoonOutragConst.corrMoon[i].dcor * d;
      let sinarg = Math.sin(arg);
      if (this.yexaaMoonOutragConst.corrMoon[i].mscor !== 0) {
        sinarg *= i1corr;
        if (
          this.yexaaMoonOutragConst.corrMoon[i].mscor === 2 ||
          this.yexaaMoonOutragConst.corrMoon[i].mscor === -2
        )
          sinarg *= i1corr;
      }
      if (this.yexaaMoonOutragConst.corrMoon[i].fcor !== 0) sinarg *= i2corr;
      lk += this.yexaaMoonOutragConst.corrMoon[i].lcor * sinarg;
    }
    for (let i = 0; i < 27; i++) {
      // outrage at length additional
      let arg =
        this.yexaaMoonOutragConst.corrMoon2[i].ml * ml +
        this.yexaaMoonOutragConst.corrMoon2[i].ms * ms +
        this.yexaaMoonOutragConst.corrMoon2[i].f * f +
        this.yexaaMoonOutragConst.corrMoon2[i].d * d;
      let sinarg = Math.sin(arg);
      lk1 += this.yexaaMoonOutragConst.corrMoon2[i].l * sinarg;
    }
