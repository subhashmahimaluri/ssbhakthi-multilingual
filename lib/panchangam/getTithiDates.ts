    ...
    try {
      ...
      if (foundTithi) {
        ...
        if (tithiTiming && tithiTiming.start && tithiTiming.end) {
          ...
          const panchangamCalendar = yexaaCalendar.calendar(
            yexaaConstant,
            calcDate,
            lat,
            lng
          ...
          const solarLongAtNewMoon = yexaaPanchangImpl.fix360(
            yexaaPanchangImpl.sun(newMoonJd) + 
            yexaaPanchangImpl.calcayan(newMoonJd)
          );
...
function calculateTithiTiming(
  ...
  startJd = refineTithiBoundary(yexaaPanchangImpl, startJd - 0.02, startJd + 0.02, startPhase, true, isLastTithi, targetTithi);
  endJd = refineTithiBoundary(yexaaPanchangImpl, endJd - 0.02, endJd + 0.02, endPhase, false, isLastTithi, targetTithi);
...
function refineTithiBoundary(
  ...
  while (endJd - startJd > 0.00001) { // Refine to about 1 second
